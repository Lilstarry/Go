...
(defun try-to-kill-by (x y color)
  (let ((en-col (color (1+ *current-move*)))
        ...
        (xm (1- x))
        (xp (1+ x)))
    (when (and (is-position-ok? x ym)
               (= (check-liberties en-col x ym nil) 0))
      (update-tmp-board)
      ...
      (when (equal en-col (aref *tmp-board* x ym))
        ...
        (update-tmp-board)
        (setf (aref *tmp-board* x y) color)))
    (when (and (is-position-ok? x yp)
               (= (check-liberties en-col x yp nil) 0))
      (update-tmp-board)
      (setf (aref *tmp-board* x y) color)
      (when (equal en-col (aref *tmp-board* x yp))
        ...
        (update-tmp-board)
        (setf (aref *tmp-board* x y) color)))
    (when (and (is-position-ok? xm y)
               (= (check-liberties en-col xm y nil) 0))
      (update-tmp-board)
      (setf (aref *tmp-board* x y) color)
      (when (equal en-col (aref *tmp-board* xm y))
        ...
        (update-tmp-board)
        (setf (aref *tmp-board* x y) color)))
    (when (and (is-position-ok? xp y)
               (= (check-liberties en-col xp y nil) 0))
      (update-tmp-board)
      (setf (aref *tmp-board* x y) color)
